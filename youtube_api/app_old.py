...
def run_spider_by_cmdline(spider, query, order, category, country, language, duration):
    from scrapy import cmdline

    install_reactor("twisted.internet.asyncioreactor.AsyncioSelectorReactor")

    argv = [
        "scrapy", "crawl",  f"{spider}",
        "-a", f'query={query}', "-a", f'order={order}',
        "-a", f'category={category}', "-a", f'country={country}',
        "-a", f'language={language}', "-a", f'duration={duration}'
    ]
    cmdline.execute(argv=argv,)

def run_spider_by_crawlprocess(spider, query, order, category, country, language, duration):
    ...
    process.crawl("search", query, order, category, country, language, duration)
    process.start(stop_after_crawl=True, install_signal_handlers=False)

def run_spider_by_crawlrunner(spider, query, order, category, country, language, duration):
    ...
    d = runner.crawl(spider, query, order, category, country, language, duration)
